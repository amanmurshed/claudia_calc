...
void handle_arithmetic(char op, Calculator& calc) {
    ...
    bool val1_is_float = std::holds_alternative<float>(val1);
    bool val2_is_float = std::holds_alternative<float>(val2);
    bool val1_is_str   = std::holds_alternative<std::string>(val1);
    bool val2_is_str   = std::holds_alternative<std::string>(val2);

    spdlog::info("val1 type: {}, val2 type: {}", val1_is_float ? "float" : "string", val2_is_float ? "float" : "string");

    if (val1_is_float && val2_is_float) {
    ...
    else if (op == '+' && val1_is_str && val2_is_str) {
        result = std::get<std::string>(val1) + std::get<std::string>(val2);
    }
    else if (op == '*' && val1_is_str && val2_is_float) {
    ...
    else if (op == '*' && val1_is_float && val2_is_str) {
        ...
        spdlog::error("Cannot perform '{}' on types {} and {}",
                      op,
                      val1_is_float ? "number" : "string",
                      val2_is_float ? "number" : "string");
        return;
    }
